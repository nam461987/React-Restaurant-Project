...
class Group extends Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (
            (this.props.group.data && !this.state.form) ||
            (this.props.group.data && this.state.form && this.props.group.data.Id !== this.state.form.Id)
        ) {
    ...
    updateFormState = () => {
        ...
        for (var i = 0; i < obj.fields.length; i++) {
            if (obj.fields[i].depend) {
                if (!this.props.options.options['options_' + obj.fields[i].field + '_array']
                    && !this.props.options.options['options_' + obj.fields[i].field + '_' + obj.fields[i].option]){
                        this.props.getOptionsByDependId(obj.fields[i].field, obj.fields[i].option, group.data[obj.fields[i].depend] );
                    }
            }
        }
        this.setState({ form: this.props.group.data })
    };
    ...
    canBeSubmitted() {
        if (this.state.isNew) {
            return this.checkRequiredFields() && !this.props.group.added &&
                !_.isEqual(this.props.group.data, this.state.form)
        }
    ...
    render() {
        const { saveGroup, addGroup } = this.props;
                        ...
                        <div className="flex flex-1 w-full items-center justify-between">
                            ...
                            <FuseAnimate animation="transition.slideRightIn" delay={300}>
                                <Button
                                    ...
                                    onClick={() => { this.state.isNew ? addGroup(form) : saveGroup(form) }}
                                >
                                    Save
                                </Button>
                            </FuseAnimate>
...
export default withReducer('options', SharedReducer)(withReducer('admin', reducer)(withStyles(styles, { withTheme: true })(withRouter(connect(mapStateToProps, mapDispatchToProps)(Group)))));
