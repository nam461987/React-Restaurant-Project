...
class ComponentTable extends Component {
    ...
    componentDidMount() {
        this.props.getGroups(this.state.page, this.state.rowsPerPage);
    }
    ...
    handleChangePage = (event, page) => {
        this.props.getGroups(page, this.state.rowsPerPage);
        this.setState({ page, selected: [] });
    };

    handleChangeRowsPerPage = event => {
        this.props.getGroups(0, event.target.value);
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        ...
        setStatus: Actions.setGroupStatus
    }, dispatch);
}

function mapStateToProps({ admin, SharedReducers }) {
    return {
        Items: admin.groups.data,
