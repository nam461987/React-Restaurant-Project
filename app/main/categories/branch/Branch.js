...
class Branch extends Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (
            (this.props.branch.data && !this.state.form) ||
            (this.props.branch.data && this.state.form && this.props.branch.data.Id !== this.state.form.Id)
        ) {
    ...
    updateFormState = () => {
        ...
        for (var i = 0; i < obj.fields.length; i++) {
            if (obj.fields[i].depend) {
                if (!this.props.options.options['options_' + obj.fields[i].field + '_array']
                    && !this.props.options.options['options_' + obj.fields[i].field + '_' + obj.fields[i].option]){
                        this.props.getOptionsByDependId(obj.fields[i].field, obj.fields[i].option, branch.data[obj.fields[i].depend] );
                    }
            }
        }
        this.setState({ form: this.props.branch.data })
    };
    ...
    updateBranchState = () => {
        ...
        if (id === 'new') {
            ...
            this.props.branch.added = false;
            this.props.newBranch();
    ...
    canBeSubmitted() {
        if (this.state.isNew) {
            return this.checkRequiredFields() && !this.props.branch.added &&
                !_.isEqual(this.props.branch.data, this.state.form)
        }
    ...
    render() {
        const { saveBranch, addBranch } = this.props;
        const { form } = this.state;
        return (
            <FusePageCarded
                classes={{
                        ...
                        <div className="flex flex-1 w-full items-center justify-between">
                            ...
                            <FuseAnimate animation="transition.slideRightIn" delay={300}>
                                <Button
                                    ...
                                    onClick={() => { this.state.isNew ? addBranch(form) : saveBranch(form) }}
                                >
                                    Save
                                </Button>
                            </FuseAnimate>
                            ...
                            <div>
                                {obj.fields.filter(f => this.state.isNew ? f.create : f.edit).map(f => {
                                    switch (f.type) {
                                        ...
                                        case "select2":
                                            {
                                                //check if admin right or Branch owner. Don't need to pick Type and Branch
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        ...
        getBranch: Actions.getBranch,
        newBranch: Actions.newBranch,
        addBranch: Actions.addBranch,
        saveBranch: Actions.saveBranch,
...
export default withReducer('options', SharedReducer)(withReducer('category', reducer)(withStyles(styles, { withTheme: true })(withRouter(connect(mapStateToProps, mapDispatchToProps)(Branch)))));
