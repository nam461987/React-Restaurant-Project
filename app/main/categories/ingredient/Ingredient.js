...
class Ingredient extends Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (
            (this.props.ingredient.data && !this.state.form) ||
            (this.props.ingredient.data && this.state.form && this.props.ingredient.data.Id !== this.state.form.Id)
        ) {
    ...
    updateFormState = () => {
        ...
        for (var i = 0; i < obj.fields.length; i++) {
            if (obj.fields[i].depend) {
                if (!this.props.options.options['options_' + obj.fields[i].field + '_array']
                    && !this.props.options.options['options_' + obj.fields[i].field + '_' + obj.fields[i].option]){
                        this.props.getOptionsByDependId(obj.fields[i].field, obj.fields[i].option, ingredient.data[obj.fields[i].depend] );
                    }
            }
        }
        this.setState({ form: this.props.ingredient.data })
    };
    ...
    updateIngredientState = () => {
        ...
        if (id === 'new') {
            ...
            this.props.ingredient.added = false;
    ...
    canBeSubmitted() {
        if (this.state.isNew) {
            return this.checkRequiredFields() && !this.props.ingredient.added &&
                !_.isEqual(this.props.ingredient.data, this.state.form)
        }
    ...
    render() {
        const { saveIngredient, addIngredient } = this.props;
        const { form } = this.state;
        return (
            <FusePageCarded
                classes={{
                        ...
                        <div className="flex flex-1 w-full items-center justify-between">
                            ...
                            <FuseAnimate animation="transition.slideRightIn" delay={300}>
                                <Button
                                    ...
                                    onClick={() => { this.state.isNew ? addIngredient(form) : saveIngredient(form) }}
                                >
                                    Save
                                </Button>
                            </FuseAnimate>
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        ...
        getIngredient: Actions.getIngredient,
        newIngredient: Actions.newIngredient,
        addIngredient: Actions.addIngredient,
        saveIngredient: Actions.saveIngredient,
...
export default withReducer('options', SharedReducer)(withReducer('category', reducer)(withStyles(styles, { withTheme: true })(withRouter(connect(mapStateToProps, mapDispatchToProps)(Ingredient)))));
