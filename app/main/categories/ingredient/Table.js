...
class ComponentTable extends Component {
    ...
    componentDidMount() {
        this.props.getIngredients(this.state.page, this.state.rowsPerPage);
    }
    ...
    handleChangePage = (event, page) => {
        this.props.getIngredients(page, this.state.rowsPerPage);
        this.setState({ page, selected: [] });
    };

    handleChangeRowsPerPage = event => {
        this.props.getIngredients(0, event.target.value);
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        ...
        setStatus: Actions.setIngredientStatus
    }, dispatch);
}

function mapStateToProps({ category, SharedReducers }) {
    return {
        Items: category.ingredients.data,
