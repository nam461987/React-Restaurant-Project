...
class Category extends Component {
    ...
    updateCategoryState = () => {
        ...
        if (id === 'new') {
            ...
            this.props.category.added = false;
    ...
    canBeSubmitted() {
        if (this.state.isNew) {
            return this.checkRequiredFields() && !this.props.category.added &&
                !_.isEqual(this.props.category.data, this.state.form)
        }
        return (this.checkRequiredFields() &&
            !_.isEqual(this.props.category.data, this.state.form)) || this.state.file != null
    };
    checkRequiredFields = () => {
        let result = false;
    ...
    submit = async () => {
        const { saveCategory, addCategory } = this.props;
        const { form } = this.state;
        if (this.state.file != null) {
            await this.uploadImage();
        }
        this.state.isNew ? addCategory(form) : saveCategory(form);
    }
...
export default withReducer('options', SharedReducer)(withReducer('category', reducer)(withStyles(styles, { withTheme: true })(withRouter(connect(mapStateToProps, mapDispatchToProps)(Category)))));
