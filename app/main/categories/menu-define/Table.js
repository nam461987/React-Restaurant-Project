...
class ComponentTable extends Component {
    ...
    componentDidMount() {
        this.props.getDefines(this.state.page, this.state.rowsPerPage, this.props.match.params);
    }
    ...
    handleSelectAllClick = event => {
        if (event.target.checked) {
            this.setState(state => ({ selected: this.state.data.Items.map(n => ({ id: n.Id, status: n.Status, menuid: n.MenuId, sizeid: n.SizeId })) }));
            return;
        }
        this.setState({ selected: [] });
    };
    ...
    handleChangePage = (event, page) => {
        this.props.getDefines(page, this.state.rowsPerPage, this.props.match.params);
        this.setState({ page, selected: [] });
    };

    handleChangeRowsPerPage = event => {
        this.props.getDefines(0, event.target.value, this.props.match.params);
                    ...
                    <Table className="min-w-xl" aria-labelledby="tableTitle">
                        ...
                        <TableBody>
                            {_.orderBy(data.Items, [
                                ...
                                .map(n => {
                                    const isSelected = this.isSelected({ id: n.Id, status: n.Status, menuid: n.MenuId, sizeid: n.SizeId });
                                        ...
                                        >
                                            <TableCell className="w-48 pl-4 sm:pl-12" padding="checkbox">
                                                <Checkbox
                                                    ...
                                                    onChange={event => this.handleCheck(event, { id: n.Id, status: n.Status, menuid: n.MenuId, sizeid: n.SizeId })}
                                                />
                                            </TableCell>
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        getDefines: Actions.getDefines,
        setStatus: Actions.setDefinestatus
    }, dispatch);
}

function mapStateToProps({ category, SharedReducers }) {
    return {
        Items: category.defines.data,
