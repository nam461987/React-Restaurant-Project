...
class ComponentTable extends Component {
    ...
    componentDidMount() {
        this.props.getSizes(this.state.page, this.state.rowsPerPage);
    }
    ...
    handleChangePage = (event, page) => {
        this.props.getSizes(page, this.state.rowsPerPage);
        this.setState({ page, selected: [] });
    };

    handleChangeRowsPerPage = event => {
        this.props.getSizes(0, event.target.value);
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        getSizes: Actions.getSizes,
        setStatus: Actions.setSizestatus
    }, dispatch);
}

function mapStateToProps({ category, SharedReducers }) {
    return {
        Items: category.sizes.data,
