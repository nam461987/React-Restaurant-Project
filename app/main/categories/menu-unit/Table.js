...
class ComponentTable extends Component {
    ...
    componentDidMount() {
        this.props.getUnits(this.state.page, this.state.rowsPerPage);
    }
    ...
    handleChangePage = (event, page) => {
        this.props.getUnits(page, this.state.rowsPerPage);
        this.setState({ page, selected: [] });
    };

    handleChangeRowsPerPage = event => {
        this.props.getUnits(0, event.target.value);
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        ...
        setStatus: Actions.setUnitstatus
    }, dispatch);
}

function mapStateToProps({ category, SharedReducers }) {
    return {
        Items: category.units.data,
