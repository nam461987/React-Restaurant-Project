...
class Unit extends Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (
            (this.props.unit.data && !this.state.form) ||
            (this.props.unit.data && this.state.form && this.props.unit.data.Id !== this.state.form.Id)
        ) {
    ...
    updateFormState = () => {
        ...
        for (var i = 0; i < obj.fields.length; i++) {
            if (obj.fields[i].depend) {
                if (!this.props.options.options['options_' + obj.fields[i].field + '_array']
                    && !this.props.options.options['options_' + obj.fields[i].field + '_' + obj.fields[i].option]){
                        this.props.getOptionsByDependId(obj.fields[i].field, obj.fields[i].option, unit.data[obj.fields[i].depend] );
                    }
            }
        }
        this.setState({ form: this.props.unit.data })
    };
    ...
    canBeSubmitted() {
        if (this.state.isNew) {
            return this.checkRequiredFields() && !this.props.unit.added &&
                !_.isEqual(this.props.unit.data, this.state.form)
        }
        return (this.checkRequiredFields() &&
            !_.isEqual(this.props.unit.data, this.state.form)) || this.state.file != null
    };
    checkRequiredFields = () => {
        let result = false;
    ...
    submit = async () => {
        const { saveUnit, addUnit } = this.props;
        const { form } = this.state;
        if (this.state.file != null) {
            await this.uploadImage();
        }
        this.state.isNew ? addUnit(form) : saveUnit(form);
    }

    render() {
        const { classes, saveUnit, addUnit } = this.props;
        const { form } = this.state;
        return (
            <FusePageCarded
                classes={{
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        ...
        newUnit: Actions.newUnit,
        addUnit: Actions.addUnit,
        saveUnit: Actions.saveUnit,
...
export default withReducer('options', SharedReducer)(withReducer('category', reducer)(withStyles(styles, { withTheme: true })(withRouter(connect(mapStateToProps, mapDispatchToProps)(Unit)))));
