...
import ComponentTable from './PriceTable';
import priceObj from './configs/price-config';
...
class Menu extends Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (
            (this.props.menu.data && !this.state.form) ||
            (this.props.menu.data && this.state.form && this.props.menu.data.Id !== this.state.form.Id)
        ) {
    ...
    updateFormState = () => {
        ...
        for (var i = 0; i < obj.fields.length; i++) {
            if (obj.fields[i].depend) {
                if (!this.props.options.options['options_' + obj.fields[i].field + '_array']
                    && !this.props.options.options['options_' + obj.fields[i].field + '_' + obj.fields[i].option]) {
                    this.props.getOptionsByDependId(obj.fields[i].field, obj.fields[i].option, menu.data[obj.fields[i].depend]);
                }
            }
        }
        this.setState({ form: this.props.menu.data })
    };
    ...
    canBeSubmitted() {
        if (this.state.isNew) {
            return this.checkRequiredFields() && !this.props.menu.added &&
                !_.isEqual(this.props.menu.data, this.state.form)
        }
        return (this.checkRequiredFields() &&
            !_.isEqual(this.props.menu.data, this.state.form)) || this.state.file != null
    };
    checkRequiredFields = () => {
        let result = false;
    ...
    submit = async () => {
        const { saveMenu, addMenu } = this.props;
        const { form } = this.state;
        if (this.state.file != null) {
            await this.uploadImage();
        }
        this.state.isNew ? addMenu(form) : saveMenu(form);
    }
        ...
        return (
            <FusePageCarded
                ...
                contentToolbar={
                    ...
                    >
                        <Tab className="h-64 normal-case" label="Menu" />
                        ...
                        <div className="p-16 sm:p-24">
                            ...
                            {tabValue === 1 && !this.state.isNew && (
                                <FusePageCarded
                                    ...
                                    header={
                                        <ComponentHeader obj={priceObj} headersearch={false} menu={form} />
                                    }
                                    content={
                                        <ComponentTable obj={priceObj} menu={form} />
                                    }
                                    innerScroll
                                />
                            )}
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        ...
        newMenu: Actions.newMenu,
        addMenu: Actions.addMenu,
        saveMenu: Actions.saveMenu,
...
export default withReducer('options', SharedReducer)(withReducer('category', reducer)(withStyles(styles, { withTheme: true })(withRouter(connect(mapStateToProps, mapDispatchToProps)(Menu)))));
