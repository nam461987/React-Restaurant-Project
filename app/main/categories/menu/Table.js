...
class ComponentTable extends Component {
    ...
    componentDidMount() {
        this.props.getMenus(this.state.page, this.state.rowsPerPage);
    }
    ...
    handleChangePage = (event, page) => {
        this.props.getMenus(page, this.state.rowsPerPage);
        this.setState({ page, selected: [] });
    };

    handleChangeRowsPerPage = event => {
        this.props.getMenus(0, event.target.value);
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        ...
        setStatus: Actions.setMenustatus
    }, dispatch);
}

function mapStateToProps({ category, SharedReducers }) {
    return {
        Items: category.menus.data,
