...
class Channel extends Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (
            (this.props.channel.data && !this.state.form) ||
            (this.props.channel.data && this.state.form && this.props.channel.data.Id !== this.state.form.Id)
        ) {
    ...
    updateFormState = () => {
        ...
        for (var i = 0; i < obj.fields.length; i++) {
            if (obj.fields[i].depend) {
                if (!this.props.options.options['options_' + obj.fields[i].field + '_array']
                    && !this.props.options.options['options_' + obj.fields[i].field + '_' + obj.fields[i].option]){
                        this.props.getOptionsByDependId(obj.fields[i].field, obj.fields[i].option, channel.data[obj.fields[i].depend] );
                    }
            }
        }
        this.setState({ form: this.props.channel.data })
    };
    ...
    updateChannelState = () => {
        ...
        if (id === 'new') {
            ...
            this.props.channel.added = false;
    ...
    canBeSubmitted() {
        if (this.state.isNew) {
            return this.checkRequiredFields() && !this.props.channel.added &&
                !_.isEqual(this.props.channel.data, this.state.form)
        }
        return (this.checkRequiredFields() &&
            !_.isEqual(this.props.channel.data, this.state.form)) || this.state.file != null
    };
    checkRequiredFields = () => {
        let result = false;
    ...
    submit = async () => {
        const { saveChannel, addChannel } = this.props;
        const { form } = this.state;
        if (this.state.file != null) {
            await this.uploadImage();
        }
        this.state.isNew ? addChannel(form) : saveChannel(form);
    }

    render() {
        const { classes, saveChannel, addChannel } = this.props;
        const { form } = this.state;
        return (
            <FusePageCarded
                classes={{
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        ...
        getChannel: Actions.getChannel,
        newChannel: Actions.newChannel,
        addChannel: Actions.addChannel,
        saveChannel: Actions.saveChannel,
...
export default withReducer('options', SharedReducer)(withReducer('category', reducer)(withStyles(styles, { withTheme: true })(withRouter(connect(mapStateToProps, mapDispatchToProps)(Channel)))));
