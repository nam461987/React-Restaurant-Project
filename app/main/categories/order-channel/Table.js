...
class ComponentTable extends Component {
    ...
    componentDidMount() {
        this.props.getChannels(this.state.page, this.state.rowsPerPage);
    }
    ...
    handleChangePage = (event, page) => {
        this.props.getChannels(page, this.state.rowsPerPage);
        this.setState({ page, selected: [] });
    };

    handleChangeRowsPerPage = event => {
        this.props.getChannels(0, event.target.value);
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        ...
        setStatus: Actions.setChannelstatus
    }, dispatch);
}

function mapStateToProps({ category, SharedReducers }) {
    return {
        Items: category.channels.data,
