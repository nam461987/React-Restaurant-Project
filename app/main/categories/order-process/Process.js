...
class Process extends Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (
            (this.props.process.data && !this.state.form) ||
            (this.props.process.data && this.state.form && this.props.process.data.Id !== this.state.form.Id)
        ) {
    ...
    updateFormState = () => {
        ...
        for (var i = 0; i < obj.fields.length; i++) {
            if (obj.fields[i].depend) {
                if (!this.props.options.options['options_' + obj.fields[i].field + '_array']
                    && !this.props.options.options['options_' + obj.fields[i].field + '_' + obj.fields[i].option]){
                        this.props.getOptionsByDependId(obj.fields[i].field, obj.fields[i].option, process.data[obj.fields[i].depend] );
                    }
            }
        }
        this.setState({ form: this.props.process.data })
    };
    ...
    updateProcessState = () => {
        ...
        if (id === 'new') {
            ...
            this.props.process.added = false;
    ...
    canBeSubmitted() {
        if (this.state.isNew) {
            return this.checkRequiredFields() && !this.props.process.added &&
                !_.isEqual(this.props.process.data, this.state.form)
        }
        return (this.checkRequiredFields() &&
            !_.isEqual(this.props.process.data, this.state.form)) || this.state.file != null
    };
    checkRequiredFields = () => {
        let result = false;
    ...
    submit = async () => {
        const { saveProcess, addProcess } = this.props;
        const { form } = this.state;
        if (this.state.file != null) {
            await this.uploadImage();
        }
        this.state.isNew ? addProcess(form) : saveProcess(form);
    }

    render() {
        const { classes, saveProcess, addProcess } = this.props;
        const { form } = this.state;
        return (
            <FusePageCarded
                classes={{
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        ...
        getProcess: Actions.getProcess,
        newProcess: Actions.newProcess,
        addProcess: Actions.addProcess,
...
export default withReducer('options', SharedReducer)(withReducer('category', reducer)(withStyles(styles, { withTheme: true })(withRouter(connect(mapStateToProps, mapDispatchToProps)(Process)))));
