...
class ComponentTable extends Component {
    ...
    componentDidMount() {
        this.props.getProcesses(this.state.page, this.state.rowsPerPage);
    }
    ...
    handleChangePage = (event, page) => {
        this.props.getProcesses(page, this.state.rowsPerPage);
        this.setState({ page, selected: [] });
    };

    handleChangeRowsPerPage = event => {
        this.props.getProcesses(0, event.target.value);
                                                                ...
                                                                default:
                                                                    {
                                                                        if (f.field == 'Color') {
                                                                            return (
                                                                                <TableCell key={index} className="w-52" component="th" scope="row" padding="none">
                                                                                    <div className={`inline text-12 p-4 rounded truncate ${n.Color}`}>
                                                                                        {n[c]}
                                                                                    </div>
                                                                                </TableCell>
                                                                            );
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        ...
        setStatus: Actions.setProcessestatus
    }, dispatch);
}

function mapStateToProps({ category, SharedReducers }) {
    return {
        Items: category.processes.data,
