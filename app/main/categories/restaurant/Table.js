...
class ComponentTable extends Component {
    ...
    componentDidMount() {
        this.props.getRestaurants(this.state.page, this.state.rowsPerPage);
    }
    ...
    handleChangePage = (event, page) => {
        this.props.getRestaurants(page, this.state.rowsPerPage);
        this.setState({ page, selected: [] });
    };

    handleChangeRowsPerPage = event => {
        this.props.getRestaurants(0, event.target.value);
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        ...
        setStatus: Actions.setRestaurantStatus
    }, dispatch);
}

function mapStateToProps({ category, SharedReducers }) {
    return {
        Items: category.restaurants.data,
