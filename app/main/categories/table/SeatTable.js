...
class SeatTable extends Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (
            (this.props.seatTable.data && !this.state.form) ||
            (this.props.seatTable.data && this.state.form && this.props.seatTable.data.Id !== this.state.form.Id)
        ) {
    ...
    updateFormState = () => {
        const { seatTable } = this.props;
        // auto get depend option list in update item mode
        for (var i = 0; i < obj.fields.length; i++) {
            if (obj.fields[i].depend) {
                if (!this.props.options.options['options_' + obj.fields[i].field + '_array']
                    && !this.props.options.options['options_' + obj.fields[i].field + '_' + obj.fields[i].option]){
                        this.props.getOptionsByDependId(obj.fields[i].field, obj.fields[i].option, seatTable.data[obj.fields[i].depend] );
                    }
            }
        }
        this.setState({ form: this.props.seatTable.data })
    };

    updateSeatTableState = () => {
        ...
        if (id === 'new') {
            ...
            this.props.seatTable.added = false;
            this.props.newSeatTable();
    ...
    canBeSubmitted() {
        if (this.state.isNew) {
            return this.checkRequiredFields() && !this.props.seatTable.added &&
                !_.isEqual(this.props.seatTable.data, this.state.form)
        }
        return (this.checkRequiredFields() &&
            !_.isEqual(this.props.seatTable.data, this.state.form)) || this.state.file != null
    };
    checkRequiredFields = () => {
        let result = false;
    ...
    submit = async () => {
        const { saveSeatTable, addSeatTable } = this.props;
        const { form } = this.state;
        if (this.state.file != null) {
            await this.uploadImage();
        }
        this.state.isNew ? addSeatTable(form) : saveSeatTable(form);
    }

    render() {
        const { classes, saveSeatTable, addSeatTable } = this.props;
        const { form } = this.state;
        return (
            <FusePageCarded
                classes={{
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        ...
        getSeatTable: Actions.getSeatTable,
        newSeatTable: Actions.newSeatTable,
        addSeatTable: Actions.addSeatTable,
        saveSeatTable: Actions.saveSeatTable,
...
function mapStateToProps({ category, options, auth }) {
    return {
        seatTable: category.table,
        options: options,
        user: auth.user
    }
}

export default withReducer('options', SharedReducer)(withReducer('category', reducer)(withStyles(styles, { withTheme: true })(withRouter(connect(mapStateToProps, mapDispatchToProps)(SeatTable)))));
