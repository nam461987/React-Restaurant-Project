...
class ComponentTable extends Component {
    ...
    componentDidMount() {
        this.props.getSeatTables(this.state.page, this.state.rowsPerPage);
    }
    ...
    handleChangePage = (event, page) => {
        this.props.getSeatTables(page, this.state.rowsPerPage);
        this.setState({ page, selected: [] });
    };

    handleChangeRowsPerPage = event => {
        this.props.getSeatTables(0, event.target.value);
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        getSeatTables: Actions.getSeatTables,
        setStatus: Actions.setSeatTablestatus
    }, dispatch);
}

function mapStateToProps({ category, SharedReducers }) {
    return {
        Items: category.tables.data,
